...
export default function ContractErrorState() {
  const activeWork = useWorkStore((state) => state.activeWork);
  const clearWork = useWorkStore((state) => state.clearWork);
  
  // Extract error message
  const errorMessage = activeWork?.conditions?.find(c => c.status === 'CONTRACT_ERROR')?.message || 'Unknown Validation Error';
  ...
  return (
    <div className="flex h-screen bg-[var(--eureka-canvas)] items-center justify-center text-[var(--eureka-text-section)]">
      <div className="flex flex-col items-center max-w-3xl space-y-6 text-center">
        <h2 className="text-3xl text-purple-500 font-light">EUREKA CONTRACT ERROR</h2>
        <div className="fabric-panel p-6 bg-[#16161c] border-purple-500 border w-full text-left overflow-auto">
          <p className="text-sm text-purple-300 font-bold mb-4">
            The Runtime returned a WorkState that does not satisfy the CanonicalWorkState contract.
          </p>
          ...
          <p className="text-xs text-[var(--eureka-text-label)] mt-6">
            Runtime authority has been preserved. The workspace explicitly rejects assumptions about invalid backend states.
          </p>
        </div>
